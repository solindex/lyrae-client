/**
 * How to use:
 * 1.) Update the lyrae group name on line 7
 * 2.) Run yarn launch-realistic-group
 * 3.) Update the lyrae group name in keeper.ts crank.ts and in the UI in useLyraeStore.ts
 */

const newGroupName = 'devnet.2';
const lyraeProgramId = '4skJ85cdxQAFVKbcGgfun8iZPL7BadVYXG3kGEGkufqA';
...
const initNewGroup = async () => {
  ...
  await execCommand(
    `yarn cli init-group ${newGroupName} ${lyraeProgramId} ${serumProgramId} ${quoteMint} ${feesVault}`,
  );
  ...
  for (let i = 0; i < FIXED_IDS.length; i++) {
    ...
    await execCommand(
      `yarn cli add-spot-market ${newGroupName} ${fids.symbol} ${fids.mint
      } --base_lot_size ${fids.baseLot} --quote_lot_size ${fids.quoteLot
      } --init_leverage ${fids.initLeverage || 5} --maint_leverage ${fids.maintLeverage || 10
    ...
    if (fids.symbol === 'BTC') {
      ...
      await execCommand(
        `yarn cli add-perp-market ${newGroupName} ${fids.symbol
        } --init_leverage ${2 * (fids.initLeverage || 5)} --maint_leverage ${2 * (fids.maintLeverage || 10)
  ...
  console.log('Succcessfully created new lyrae group.');
};
