...
import { msrmMints, LyraeClient, I80F48 } from '../src';
import LyraeGroup from '../src/LyraeGroup';
import LyraeAccount from '../src/LyraeAccount';

export const LyraeProgramId = new PublicKey(
...
export async function addSpotMarketToLyraeGroup(
  client: LyraeClient,
  payer: Account,
  ...
  const oraclePk = await createOracle(client.connection, LyraeProgramId, payer);
  await client.addOracle(lyraeGroup, oraclePk, payer);
...
export async function addSpotMarketsToLyraeGroup(
  client: LyraeClient,
  payer: Account,
  ...
  for (let i = 0; i < mints.length - 1; i++) {
    ...
    await addSpotMarketToLyraeGroup(
      client,
      payer,
      lyraeGroup,
      mint,
  ...
  return await client.getLyraeGroup(lyraeGroupPk);
}
...
export async function getNodeBank(
  ...
  let rootBanks = await lyraeGroup.loadRootBanks(client.connection);
...
export async function cachePrices(
  ...
  for (let oracleIndex of oracleIndices) {
    pricesToCache.push(lyraeGroup.oracles[oracleIndex]);
  }
...
export async function cacheRootBanks(
  ...
  for (let rootBankIndex of rootBankIndices) {
    rootBanksToCache.push(lyraeGroup.tokens[rootBankIndex].rootBank);
  }
...
export async function performDeposit(
  ...
  await client.deposit(
    ...
    lyraeAccount,
    payer,
    lyraeGroup.tokens[tokenIndex].rootBank,
    nodeBank.publicKey,
    nodeBank.vault,
    tokenAccountPk,
    quantity,
  );
  return await client.getLyraeAccount(
