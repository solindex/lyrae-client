/**
 * How to use:
 * 1.) Update the lyrae group name on line 6
 * 2.) Run yarn launch-group
 * 3.) Update the lyrae group name in keeper.ts crank.ts and in the UI in useLyraeStore.ts
 */
const newGroupName = 'lyrae_test_v4';
const lyraeProgramId = '5fP7Z7a87ZEVsKr2tQPApdtq83GcTW4kz919R6ou5h5E';
...
const initNewGroup = async () => {
  ...
  const quoteMint = FIXED_IDS.find((id) => id.symbol === 'USDC')
    ?.mint as string;
  await execCommand(
    `yarn cli init-group ${newGroupName} ${lyraeProgramId} ${serumProgramId} ${quoteMint}`,
  );
