...
import { loadTestLyraeGroup, loadTestLyraeNodeBank, loadTestLyraeRootBank } from './testdata';
...
describe('Root Banks and Interest Rates', async () => {
  ...
  describe('root bank', async () => {
    it('loading the root bank from a file should load the correct values', async () => {
      ...
      const rootBank: RootBank = loadTestLyraeRootBank(`${prefix}/root_bank0.json`)
  ...
  describe('interest rates', async () => {
    it('BTC root bank should return correct interest rate', async () => {
      ...
      const rootBank: RootBank = loadTestLyraeRootBank(`${prefix}/btc_root_bank.json`)
      const nodeBank: NodeBank = loadTestLyraeNodeBank(`${prefix}/btc_node_bank.json`)
      rootBank.nodeBankAccounts = [nodeBank]

      expect(
        rootBank.getBorrowRate(lyraeGroup)
          .toString()
      ).to.equal("0.0060962691428017024");

      expect(
        rootBank.getDepositRate(lyraeGroup)
          .toString()
    ...
    it('USDC root bank should return correct interest rate', async () => {
      const prefix = "./testdata/tokenbank"
      const lyraeGroup: LyraeGroup = loadTestLyraeGroup(`${prefix}/group.json`)
      const rootBank: RootBank = loadTestLyraeRootBank(`${prefix}/usdc_root_bank.json`)
      const nodeBank: NodeBank = loadTestLyraeNodeBank(`${prefix}/usdc_node_bank.json`)
      rootBank.nodeBankAccounts = [nodeBank]
