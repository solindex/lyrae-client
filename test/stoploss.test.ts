...
async function testStopLoss() {
  ...
  const makerPk = await testGroup.client.initLyraeAccount(lyraeGroup, payer);
  console.log('Created Maker:', accountPk.toBase58());
  await sleep(sleepTime);
  const maker = await testGroup.client.getLyraeAccount(
    makerPk,
    lyraeGroup.dexProgramId,
  );
  ...
  console.log(
    'health',
    account.getHealthRatio(lyraeGroup, cache, 'Maint').toString(),
  );
